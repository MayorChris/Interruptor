...
export default function CustomizedMenus({ label, items }) {
  ...
  items.map((_, index) => {
    clickValues[index.toString()] = false;
    return null;
  });
    ...
    <div>
      ...
      >
        {items.map((item, index) => (
          <MenuItem onClick={handleChanges} id={index} disableRipple>
            {clickItem[index] == false && <CheckBoxOutlineBlankIcon />}
            {clickItem[index] == true && <CheckIcon />}
            {item}
          </MenuItem>
        ))}
      </StyledMenu>
